...
interface CachedPrices {
  ...
  yearly: { priceId: string; amount: number; currency: string; monthlyEquivalent: number; discountPercent: number } | null;
  cachedAt: number;
}
...
export class BillingService {
  ...
  async getProductWithPrices() {
    ...
    if (this.pricesCache && Date.now() - this.pricesCache.cachedAt < this.CACHE_TTL) {
      const product = await stripe.products.retrieve(productId);
      return {
        product: { id: product.id, name: product.name, description: product.description, images: product.images },
        prices: { monthly: this.pricesCache.monthly, yearly: this.pricesCache.yearly },
      };
    }
    ...
    for (const price of prices.data) {
      if (price.recurring?.interval === 'month') {
        monthly = { priceId: price.id, amount: price.unit_amount || 0, currency: price.currency };
      } else if (price.recurring?.interval === 'year') {
        yearly = { priceId: price.id, amount: price.unit_amount || 0, currency: price.currency, monthlyEquivalent: 0, discountPercent: 0 };
      }
    }
    ...
    if (monthly && yearly) {
      ...
      yearly.discountPercent = Math.round((1 - yearly.amount / monthlyTotal) * 100);
    }
  ...
  async createCheckoutSession(userId: string, email: string, name: string, interval: 'month' | 'year') {
    const stripe = this.ensureStripe();
    ...
    const price = interval === 'month' ? prices.prices.monthly : prices.prices.yearly;
  ...
  private async handleCheckoutCompleted(session: Stripe.Checkout.Session) {
    ...
    const subscription = await this.ensureStripe().subscriptions.retrieve(subscriptionId) as any;
  ...
  private async handleSubscriptionDeleted(subscription: Stripe.Subscription) {
    ...
    this.logger.log(`Subscription cancelled for user ${subscriptionRecord.userId}`);
  }
  ...
  private mapStripeStatus(status: string): 'ACTIVE' | 'PAST_DUE' | 'CANCELLED' | 'UNPAID' {
    switch (status) {
